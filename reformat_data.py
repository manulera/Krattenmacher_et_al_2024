...
for f in ["single","AP"]:
    outdict = dict()
    for dir in os.listdir('experimental_data'):
        if not os.path.isdir('experimental_data/'+dir) or dir[0]=='.':
            continue
        
        key = dir[8:]
        data = np.genfromtxt(f'experimental_data/{dir}/{f}.csv',delimiter=',',skip_header=1)
        outdict[key] = data[:,1]
    ...
    print(df)
    df.to_csv('experimental_data/data_'+f+'.csv')
