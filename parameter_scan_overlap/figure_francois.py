...
model_3 = loadSimulationResults('runs1')

models = [model_3]
names = ['Model 3, N=3, kon XX']
...
for model, name, ls, chosen_val in zip(models,names,linestyles,chosen_vals):
    sns.lineplot(x='accumulation_end_fit',y='shrinking_speed_steady_state',data=model,ls=ls,hue='cooperativity', label=name)
    d = model[model["kon"]==chosen_val]
...
for model, name, ls, chosen_val in zip(models,names,linestyles,chosen_vals):
    sns.lineplot(x='accumulation_timescale',y='shrinking_speed_steady_state',data=model,ls=ls,hue='cooperativity', label=name)
    d = model[model["kon"]==chosen_val]
